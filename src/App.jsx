...
function Spinner({ label = 'Loading Zeno...' }) {
  return (
    <div style={{ minHeight:'100vh', background:'var(--cream)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
      <div style={{ width:36, height:36, borderRadius:'50%', border:'3px solid var(--border)', borderTopColor:'var(--sage)', animation:'spin 0.8s linear infinite' }} />
      <p style={{ fontFamily:'Fraunces, serif', fontSize:16, color:'var(--ink-soft)', fontWeight:400 }}>{label}</p>
...
export default function App() {
  ...
  useEffect(() => { if (authed) init() else setLoading(false) }, [authed])
